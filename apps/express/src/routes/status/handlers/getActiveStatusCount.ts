...
import { fail, ok, getActiveCount } from "@ccpilot/domain";

export const getActiveStatusCount: ContextHandler = async (req, res) => {
  const { ctx } = res.locals;
  const userStatusRepo = ctx.repos.status;

  const result = await getActiveCount(userStatusRepo);
